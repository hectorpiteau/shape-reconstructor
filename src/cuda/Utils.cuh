...
CUDA_DEV inline unsigned int SparseVolumeGetDataIndex(ivec3 coords, SparseVolumeDescriptor *volume) {
    ...
    auto s0_cell = volume->stage0[STAGE0_INDEX(s0_coords.x, s0_coords.y, s0_coords.z, volume->initialResolution)];
    ...
    if (s0_cell.index >= volume->stage0Size) return INF;
...
//    auto re= (float)SparseVolumeGetDataIndex(vec3(min.x, min.y, min.z), volume);
//    if(re == INF) return vec4(1.0, 1.0, 0.0, 1.0);
//    if(re == 0) return vec4(0.0, 0.0, 1.0, 1.0);
//    return vec4(re / (128.0f*128.0f*192.0f),
//                re / (128.0f*128.0f*192.0f),
//                re / (128.0f*128.0f*192.0f*4.0f*4.0f*4.0f), 1.0);
//    return vec4(min.x / 128.0f, min.y / 128.0f,  min.z / 192.0f, 1.0);
