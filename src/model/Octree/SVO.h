...
void initialize_svo(int32_t* nodes, size_t length, int current_depth, int max_depth, int& current_index) {
    std::vector<int32_t>& far_pointers;
    if (current_depth > max_depth) return; // Base case for recursion
    ...
    if (children_start_index + children_count >= std::pow(2, 15)) {
        ...
        nodes[base_index] = children_start_index;
    }
